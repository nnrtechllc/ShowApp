...
struct PostCardView: View {
 ...
 //       .hAlign(.leading)
        .overlay(alignment: .topTrailing, content: {
            // Displaying Delete Button (if its author of that post)
            if post.userUID == userUID{
                Menu {
                    Button("Delete Posts",role: .destructive,action: deletePost)
                } label: {
    ...
    func likePost(){
        Task{
            ...
            if post.likedIDs.contains(userUID){
               ...
               // Adding User ID to liked Arrray and removing our ID from Disliked Array (if added in prior)
               try await Firestore.firestore().collection("Posts").document(postID).updateData([
